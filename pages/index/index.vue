<template>
	<view class="dcbox">
		<image @click="tonext" class="bgimg" src="../../static/bgs.png" mode=""></image>
	...
	<popupFowTip v-model="showTip" title="填写收货地址" desc="请使用微信扫码二维码填写收货信息" :src="ercode"></popupFowTip>
		<view @tap="goBack" class="goback flex-center" style="position: absolute;left: 20rpx;top: 40rpx;background: rgba(0,0,0,.4);width: 60rpx;height: 60rpx;border-radius: 50%;">
			<u-icon style="margin-bottom: 2rpx;" name="arrow-left" size="38" color="#fff"></u-icon>
		</view>
		<view class="cbox">
			<view class="tabs_box">
			...
			<view v-show="curIndex == 0"  class="scview_dc">
				<view class="flex flex-wrap flex-justify-between bbx" style="padding-bottom: 122rpx;">
					<view class="item flex-column" v-for="(item,index) in goodsList" :key="index"
						@tap="tapToDec(item.type, item, index,1)">
						<u-image width="322" height="322"
							:src="item.pic ? item.pic :'/static/noPic/360.png'">
						...
						<view class="p_sell flex-justify-between">
							...
							<view class="" style="margin-right: 16rpx;">
								<number-box :txtStyle="{color:'#333333',fontSize:'32rpx',height:'48rpx',width:'48rpx'}"
									btnWidth="48" btn-height="48" @remove="removeGood(item,index)" @add="(e)=>{addGood(e,item,index)}"
				...
				<view class="flex flex-wrap flex-justify-between bbx" style="padding-bottom: 122rpx;">
					<view class="item flex-column" v-for="(item,index) in serverList" :key="index"
						...
						<view class="p_sell flex-justify-between">
							...
							<view class="yishou">
								<u-button @click="handleAddGood(item,index)" border-radius="6rpx" :hair-line="false" ripple :custom-style="customStyle">立即购买</u-button>
							</view>
							<!-- <view class="" style="margin-right: 16rpx;">
								<number-box :txtStyle="{color:'#333333',fontSize:'32rpx',height:'48rpx',width:'48rpx'}"
		...
		<view v-if="curIndex == 0" class="gcart flex-items-center flex-justify-between" id="fixBox" :class="{'isShowCart':watchList.length}">
		...
		<u-popup v-model="showCart" mode="bottom" border-radius="17" height="574rpx" z-index="300">
			<view class="cartPop" style="padding: 32rpx 20rpx 120rpx;">
				<view class="actions flex flex-justify-between flex-items-center" style="margin-right:18rpx;margin-bottom: 34rpx;">
					<view class="flex flex-items-center" @tap="handleSelectAll()">
						<image v-if="!allCheck" style="width: 40rpx;height: 40rpx;" src="../../static/img/noselect.png" mode=""></image>
						<image v-else style="width: 40rpx;height: 40rpx;" src="../../static/img/selected.png" mode=""></image>
						<view class="quanxuan"  >
							全选
						</view>
					</view>
				...
				<view class="watch_list flex-items-center" v-for="(item,index) in watchList" :key="index">
					<image v-if="!item.checked" @tap="handleCheck(index)" style="width: 40rpx;height: 40rpx;margin-right: 19rpx;flex-shrink: 0;" src="../../static/img/noselect.png" mode=""></image>
					<image v-if="item.checked" @tap="handleCheck(index)"  style="width: 40rpx;height: 40rpx;margin-right: 19rpx;flex-shrink: 0;" src="../../static/img/selected.png" mode=""></image>
					<image style="width: 158rpx;height: 158rpx;margin-right: 20rpx;border-radius: 10rpx;flex-shrink: 0;" :src="item.pic ? item.pic + '?x-oss-process=image/resize,m_lfit,h_500,w_500' : '' || '/static/noPic/360.png'" mode=""></image>
					<view class="hw_gd flex-column flex-justify-between">
						...
						<view class="pad_goods flex-items-center flex-justify-between">
							...
							<number-box :txtStyle="{color:'#333333',fontSize:'32rpx',height:'48rpx',width:'48rpx'}"
								btnWidth="48" btn-height="48" @remove="removeGoodCart(item,index)" @add="(e)=>{addGoodCart(item,index)}"
								:num="item.selectedNum"></number-box>
						</view>
					</view>
				</view>
		...
		<!-- <updateApp :show="isShowUpdate" :isForce="isForce" :apkUrl="apkUrl" @close="hideModal" :updateContent="updateContent"></updateApp> -->
	</view>
</template>
		...
		methods: {
			...
			addGoodsSku(chooseItem,selectedNum,flag){
				...
				if(indexfind != -1){
					...
					this.watchList.push({...chooseItem,selectedNum:selectedNum,checked:true})
				}
			...
			addGood(e,item,i) {
				...
				if(finindex!= -1){
					...
					this.goodsList[i].selectedNum = this.goodsList[i].selectedNum + 1
					this.watchList.push({...cloneItem,checked:true})
					console.log(this.watchList);
				}
				if(e){
...
<style lang="scss" scoped>
	...
	.scass{
		transition: all .3s ease;width: 78rpx;height: 78rpx;
		transition-delay: .3s;
	}
	.scales{
		transform: scale(2);
