<template>
	...
	<view class="mix-refresh-content" :style="{
				...
				background: 'linear-gradient('+bgc+' 200rpx, #f1f1f1 0)'
		...
		<view class="mix-refresh-content" :style="{
			...
			<!-- 下拉刷新 -->
			<view v-if="showIcon" class="mix-loading-wrapper"  :style="{'top':top2+'rpx','background':bgc}">
				<!-- <image 
			...
			src="../../static/star_b.png"
				...
				<view style="font-size: 24rpx;" :style="{'color':color2}">{{text}}</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			startPulldownRefresh() {
				...
				this.pageDeviation = uni.upx2px(120);
				this.$emit('refresh');
			},
			//结束下拉刷新
			endPulldownRefresh() {
