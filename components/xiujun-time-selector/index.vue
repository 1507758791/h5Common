<template>
	<view class="container" @touchmove.stop.prevent=''>
		...
		<view class="sure_btn" @click="close">确定</view>
	</view>
</template>
...
export default {
	...
	created() {
		...
		//默认选中的日期
		// (${this.dateArr[0]['week']})
		console.log(this.dateArr[0],222)
		this.selectDate = `${this.dateArr[0]['year']}-${this.dateArr[0]['date']} `;
	...
	methods: {
		selectDateEvent(index, item) {
			...
			// ${this.dateArr[index]['date']}(${this.dateArr[index]['week']})
			this.dateActive = index;			
			this.selectDate = `${this.dateArr[0]['year']}-${this.dateArr[index]['date']} `;
