	...
	<view class="calendar-content" v-if="show">
		...
		<view class="calendar-body">
			<view class="calendar-header">
				...
				<view :class="{
					'not-confirm': true,
					'confirm': (pickerArray[0] != null && pickerArray[1] == null)
				}"
				 @click="savea">当天</view>
				<view class="title" style="width: 380rpx;">
					<!-- 选择日期 -->
					...
					<block v-for="(v, i) in calendarEmptyTempArray" :key="i">
						...
						<view class="calendar-days-content">
							...
							<view class="calendar-day-content" v-for="(d, i) in v[4]" :key="i" @click="picker([v[3], v[2], i+1])">
								<view :class="{
									'calendar-day': true,
									'calendar-disable-day': `${v[3]}/${prefixZero(v[2])}/${prefixZero(i+1)}`> todayTrue,
									'calendar-today': `${v[3]}/${prefixZero(v[2])}/${prefixZero(i+1)}` == todayTrue,
	...
	export default {
		...
		methods: {
			get3MonthBefor() {
				...
				resultDate = year + '-' + month.slice(-2) + '-01';
				return resultDate;
			},
			getCalendar() {
				...
				var datea = new Date();
				// var yeara = ((datea.getMonth() + 3) > 12 ? '' : datea.getFullYear() - 1);
				var beforeDate = this.get3MonthBefor()
                console.log(beforeDate,44567)
				...
				this.todayTrue = this.getDate(datea)
			...
			savea(e) {
				this.pickerArray = [this.pickerArray[0],this.pickerArray[0]]
				console.log(this.pickerArray,this.pickerArray[0],888,this.pickerArray[1])
				
				// if (this.pickerArray.length == 1) {
				// 	this.$emit('change', this.dateScope(this.pickerArray[0], this.pickerArray[0]))
				// 	this.show = false
				// 	if (this.autoClear) {
				// 		this.clear()
				// 	}
